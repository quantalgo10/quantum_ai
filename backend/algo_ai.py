...
def get_data(symbol, period='1d', interval='5m'): # Change Interval Here 1,2,5,15
...
class AlgoAI:
    def __init__(self):
        ...
        self.strategies = {
            ...
            'Volume Analysis': self.volume_analysis_strategy,
            'Pattern Recognition': self.pattern_recognition_strategy
        }
    ...
    def display_dashboard(self):
        """Main dashboard for Algo AI"""
        st.title("🎯 Algo AI Trading")
    ...
    def display_strategy_builder(self):
    ...
    def mean_reversion_strategy(self, data):
        ...
        try:
            ...
            data['BB_middle'] = ta.trend.sma_indicator(data['Close'], window=20)
            data['BB_upper'] = data['BB_middle'] + 2 * data['Close'].rolling(window=20).std()
            data['BB_lower'] = data['BB_middle'] - 2 * data['Close'].rolling(window=20).std()
            
            # Generate signals
            data['Signal'] = 0
            data.loc[data['Close'] > data['BB_upper'], 'Signal'] = -1  # Sell signal
            data.loc[data['Close'] < data['BB_lower'], 'Signal'] = 1   # Buy signal
    ...
    def breakout_strategy(self, data):
        ...
        try:
            ...
            # Generate signals
            data['Signal'] = 0
            data.loc[data['Close'] > data['High_20'], 'Signal'] = 1    # Breakout above resistance
            data.loc[data['Close'] < data['Low_20'], 'Signal'] = -1    # Breakdown below support
    ...
    def volume_analysis_strategy(self, data):
        """Implement volume analysis strategy"""
        try:
            ...
            # Generate signals
            data['Signal'] = 0
            data.loc[(data['Volume'] > 2 * data['Volume_SMA']) & (data['Price_ROC'] > 0), 'Signal'] = 1
            data.loc[(data['Volume'] > 2 * data['Volume_SMA']) & (data['Price_ROC'] < 0), 'Signal'] = -1
            
            return data
        except Exception as e:
            self.logger.error(f"Error in volume analysis strategy: {str(e)}")
            return None

    def pattern_recognition_strategy(self, data):
        """Implement pattern recognition strategy"""
        try:
            # Calculate candlestick patterns
            data['Doji'] = ta.candlestick.doji(data['Open'], data['High'], data['Low'], data['Close'])
            data['Hammer'] = ta.candlestick.hammer(data['Open'], data['High'], data['Low'], data['Close'])
            data['Shooting_Star'] = ta.candlestick.shooting_star(data['Open'], data['High'], data['Low'], data['Close'])
            
            # Generate signals
            data['Signal'] = 0
            data.loc[data['Hammer'] == 1, 'Signal'] = 1
            data.loc[data['Shooting_Star'] == 1, 'Signal'] = -1
            
            return data
        except Exception as e:
            self.logger.error(f"Error in pattern recognition strategy: {str(e)}")
            return None

    def display_backtesting(self):
        ...
        if st.button("Run Backtest"):
            self.run_backtest(symbol, start_date, end_date, timeframe)

    def display_live_trading(self):
    ...
    def display_settings(self):
        ...
        self.display_risk_settings()
        
        # Trading parameters
        self.display_trading_settings()
