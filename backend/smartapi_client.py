...
class SmartAPIClient:
    ...
    def get_order_book(self):
        ...
        try:
            ...
            if orders.get('status'):
                return pd.DataFrame(orders['data'])
            return pd.DataFrame()
        except Exception as e:
    ...
    def place_order(self, symbol, side, quantity, order_type='MARKET'):
        ...
        try:
            ...
            order = self.smart_api.placeOrder(params)
            return order.get('status', False)
    ...
    def get_token(self, symbol):
        """Get trading token for symbol"""
        try:
            # Add your symbol to token mapping logic here
    ...
    def get_rms_limits(self):
        ...
        try:
            ...
            self.logger.error("Error fetching RMS limits: %s", str(e))
            return None

    def get_candle_data(self, exchange, token, interval, from_date, to_date):
        ...
        try:
            ...
            self.logger.error("Error fetching candle data: %s", str(e))
            return pd.DataFrame()

    def execute_paper_trade(self, signal, price, quantity, symbol):
        """Execute paper trade based on signal"""
        try:
            if signal in ['BUY', 'SELL']:
                strike = int(price)  # Get nearest strike
                option_type = 'CE' if signal == 'BUY' else 'PE'
                option_symbol = f"{symbol} {datetime.now().strftime('%d %b').upper()} {strike} {option_type}"
                ...
                if option_type == 'CE':
                    new_trade.stop_loss = price * (1 - sl_percent)
                    new_trade.target1 = price * (1 + target1_percent)
                    new_trade.target2 = price * (1 + target2_percent)
                else:  # PE
                    new_trade.stop_loss = price * (1 + sl_percent)
                    new_trade.target1 = price * (1 - target1_percent)
                    new_trade.target2 = price * (1 - target2_percent)
                ...
                self.paper_trades.append(new_trade)
                return f"Executed {signal} paper trade: {quantity} {option_symbol} @ ₹{price}"
            
            return "No trade executed"
            
        except Exception as e:
            self.logger.error("Paper trade execution error: %s", str(e))
    ...
    def get_paper_trades(self):
        ...
        try:
            ...
            self.logger.error("Error getting paper trades: %s", str(e))
            return pd.DataFrame()
    ...
    def get_paper_trading_stats(self):
        ...
        try:
            stats = {
                'Total Trades': len(self.paper_trades),
                'Open Trades': len([t for t in self.paper_trades if t.status == 'OPEN']),
                'Closed Trades': len([t for t in self.paper_trades if t.status == 'CLOSED']),
                'Total P&L': sum(t.pnl for t in self.paper_trades if t.status == 'CLOSED'),
                'Current Balance': self.paper_balance,
                'Win Rate': 0.0
            }
            ...
            if closed_trades:
                ...
                stats['Win Rate'] = (winning_trades / len(closed_trades)) * 100
                
            return stats
        except Exception as e:
            self.logger.error("Error calculating paper trading stats: %s", str(e))
            return {}
...
def initialize_smartapi():
    ...
    try:
        ...
        smartapi_client = SmartConnect(api_key, client_id, password, totp_key)
